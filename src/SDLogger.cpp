...
void recoverTmpFile() {
    ...
    if (binFile.read(&count, 2) != 2 || count != DATA_DIM) {
        error("Please delete existing "
                      TMP_FILE_NAME);
    }
    Serial.println(F("\nRecovering data in tmp file "
                             TMP_FILE_NAME));
    uint32_t bgnBlock = 0;
...
void initSD(void) {
    ...
    if (sd.exists(TMP_FILE_NAME)) {
        Serial.println(F("\nType 'Y' to recover existing tmp file "
                                 TMP_FILE_NAME));
        while (!Serial.available()) {
            SysCall::yield();
        }
        if (Serial.read() == 'Y') {
            ...
            error("'Y' not typed, please manually delete "
                          TMP_FILE_NAME);
        }
    }
